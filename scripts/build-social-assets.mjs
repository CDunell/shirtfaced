...
const [lockupSrc, wordmarkSrc, smileySrc] = await Promise.all([
  readFile("DEV/logo-lockup.svg", "utf8"), readFile("DEV/shirtfaced.svg", "utf8"), readFile("DEV/smiley.svg", "utf8")
]);
const b64 = (s) => Buffer.from(s).toString("base64");
const LOCKUP=`data:image/svg+xml;base64,${b64(lockupSrc)}`, WORDMARK=`data:image/svg+xml;base64,${b64(wordmarkSrc)}`, SMILEY=`data:image/svg+xml;base64,${b64(smileySrc)}`;
function defs(){return `<defs><mask id="lockupMask" maskUnits="userSpaceOnUse"><image href="${LOCKUP}" width="100%" height="100%" preserveAspectRatio="xMidYMid meet"/></mask><mask id="wordmarkMask" maskUnits="userSpaceOnUse"><image href="${WORDMARK}" width="100%" height="100%" preserveAspectRatio="xMidYMid meet"/></mask><mask id="smileyMask" maskUnits="userSpaceOnUse"><image href="${SMILEY}" width="100%" height="100%" preserveAspectRatio="xMidYMid meet"/></mask></defs>`}
function svg(w,h,body,bg="none"){return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">${defs()}${bg!=="none"?`<rect width="100%" height="100%" fill="${bg}"/>`:""}${body}</svg>\n`}
...
const line=(x1,y1,x2,y2,c=COLORS.paper,sw=3,o=1)=>`<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${c}" stroke-width="${sw}" opacity="${o}"/>`;
const rect=(x,y,w,h,c,rx=0,o=1,stroke="none",sw=0)=>`<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="${rx}" fill="${c}" opacity="${o}" stroke="${stroke}" stroke-width="${sw}"/>`;
...
// V3 — environment variants. Same geometry, contrast changes with the content.
// LIGHT: beach, daylight 4WD, camping, bright streets.
...
// DARK: explicit equivalents for predictable naming in editing templates.
...
// ADAPTIVE: backing plate survives party flash, servo, crowds, mixed sun/shadow and changing video exposure.
...
// Minimal corner marks for photographs that should remain almost completely clean.
...
for(const [relative,content] of assets){const path=join(OUT,relative);await mkdir(dirname(path),{recursive:true});await writeFile(path,content,"utf8")}
const manifest={generatedAt:new Date().toISOString(),palette:COLORS,sourceAssets:["DEV/logo-lockup.svg","DEV/shirtfaced.svg","DEV/smiley.svg"],variants:{light:"bright/daylight imagery",dark:"night/dark imagery",adaptive:"mixed or changing contrast"},files:[...assets.keys()]};
