 ...
 * Existing cached images are preserved and all acquired files carry provenance.
 */
...
const CONCURRENCY = Math.max(1, Number(process.env.VINTAGE_CONCURRENCY || 6));
...
async function collectRenderedEbay(page, record) {
  ...
  try {
    ...
    return uniqueUrls([
      ...network,
      ...dom.map(normaliseEbayImage),
      ...embedded.map(normaliseEbayImage),
    ]).slice(0, MAX_IMAGES);
  } finally {
...
async function collectIndexedImages(page, record) {
  const query = encodeURIComponent(
    `\"${record.listing_id}\" ebay ${record.brand || ''} ${record.title || ''}`.slice(0, 450)
  );
  ...
  return uniqueUrls(
    urls.filter((url) => !/googlelogo|gstatic\.com\/images\/branding|favicon/i.test(url))
  ).slice(0, MAX_IMAGES);
}
...
async function saveImage(context, dir, url, index, sourceKind) {
  try {
    const response = await context.request.get(url, {
      timeout: 25000,
      headers: {
        referer: sourceKind === 'ebay-rendered' ? 'https://www.ebay.com/' : 'https://www.google.com/',
      },
    });
...
for (const file of files) {
  ...
  for (const line of text.split(/\n+/)) {
    ...
    try {
      ...
      if (id && record.sold === true && !records.has(id)) records.set(id, record);
    } catch {}
  }
}
...
let candidates = [...records.entries()];
if (MAX_RECORDS) candidates = candidates.slice(0, MAX_RECORDS);
let cursor = 0;
let attempted = 0;

async function processRecord(id, record) {
  const dir = path.join(ROOT, id);
  await mkdir(dir, { recursive: true });
  if ((await existingImageNames(dir)).length) return;
  attempted += 1;
  ...
  try {
    ...
    await writeFile(
      ...
      JSON.stringify({ ...record, stored_image_count: index }, null, 2)
    );
...
async function worker() {
  while (true) {
    ...
    const [id, record] = candidates[index];
    await processRecord(id, record).catch((error) => {
      console.error(`listing ${id} failed: ${error?.message || error}`);
    });
    await sleep(80);
  }
}
