...
const FIELD = [2, 7, 4, 9, 1, 6, 3, 8, 5, 10, 0, 11, 4, 2, 8, 7, 3, 10, 5, 1, 6, 9, 11, 0];
  ...
  return (
    <section className="bg-ink text-paper">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="relative overflow-hidden border-y border-paper/15 bg-paper px-4 py-5 text-ink">
          <div className="grid grid-cols-6 gap-x-3 gap-y-5 opacity-75">
            {FIELD.map((index, i) => {
              ...
              const isTarget = i === 10;
              return (
                <div key={`${c.slug}-${i}`} className={`flex aspect-square items-center justify-center rounded-full ${isTarget ? "bg-grit-green/70" : ""}`}>
        ...
        <div className="grid grid-cols-[1fr_auto] items-center gap-5 px-4 py-7 sm:px-6">
          <div>
            <p className="display text-[24px] uppercase leading-none">found one!</p>
            <p className="mt-1 text-[12px] font-bold text-paper/65">That&apos;s {target.name}.</p>
            <Link href={`/products/${target.slug}-tee`} className="press mt-4 inline-flex rounded-md bg-grit-green px-4 py-3 text-[11px] font-black uppercase text-ink">See {target.name}&apos;s stuff</Link>
          </div>
          {/* eslint-disable-next-line @next/next/no-img-element */}
