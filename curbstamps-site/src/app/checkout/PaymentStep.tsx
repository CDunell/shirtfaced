...
function NotConnected({ total }: { total: number }) {
  return (
    <div className="mt-8 rounded-card border-2 border-dashed border-ink/20 px-5 py-6 text-center">
      ...
      <p className="mx-auto mt-2 max-w-[40ch] text-[14px] leading-relaxed text-grey-dark">
        No card form here on purpose — no processor is wired up yet, so nothing could be
        charged. Once Stripe test (or live) keys are set, payment happens right here in
        Stripe&apos;s own embedded form.
      </p>
      <button
        type="button"
        disabled
        className="mt-5 inline-flex h-14 w-full cursor-not-allowed items-center justify-center gap-2 rounded-full bg-grit-pink text-[16px] font-extrabold text-ink opacity-50"
      >
...
export function PaymentStep({ request, total }: { request: CheckoutRequest; total: number }) {
  ...
  if (error) {
    return (
      <div className="mt-8 rounded-card border-2 border-grit-orange/40 bg-grit-orange/10 px-5 py-4 text-[14px] text-grit-orange">
        {error}
      </div>
    );
  }
  ...
  if (!clientSecret || !orderId || !stripePromise) {
    return <div className="skeleton mt-8 h-48 rounded-card" />;
  }
...
function PaymentForm({ orderId, total }: { orderId: string; total: number }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-4">
      <PaymentElement />
      {error && (
        <p role="alert" className="text-[13px] font-semibold text-grit-orange">
          {error}
        </p>
      )}
      <button
        ...
        className="press flex h-14 w-full items-center justify-center gap-2 rounded-full bg-grit-pink text-[16px] font-extrabold text-ink disabled:opacity-50"
      >
        <IconLock className="h-5 w-5" />
