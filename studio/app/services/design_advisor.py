...
def render_generation_prompt(direction: DesignDirection, phrase: str = "") -> str:
    ...
    return (
        ...
        "If the concept calls for lettering, the brand name is 'Shirtfaced' -- the same name "
        "on every design, not a new invented one each time. Design it as a considered piece "
        "of type, not a caption: letter it in the same hand as the rest of the "
        "illustration -- matching its line weight, texture and construction (hand-carved, "
        "screen-printed, hand-lettered, whatever the piece itself is drawn in) -- and give it "
        "real presence in the composition, not a small afterthought line dropped underneath "
        "in a plain default font. Do not render this description's own wording as the printed "
        "text, and do not depict any real trademarked logo or brand name belonging to anyone "
        "else. If the concept calls for a character or mascot, "
