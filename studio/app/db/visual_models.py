...
class VisualAsset(Base, TimestampMixin):
    ...
    # Verified by default, which is the opposite of the element archive and for a
    # reason: everything in this library is invented here. The worlds, the cast,
    # the locations, the masters and the coverage are all generated by the owner,
    # there are no photographs of real people in it, and there is no third party
    # to have a claim. Owner's ruling, 17 August 2026.
    #
    # ``app/archive/`` keeps UNVERIFIED as its default and should: that library
    # holds found material whose terms genuinely are unknown until someone reads
    # them. Same enum, different question.
    #
