...
.agent-panel{margin:18px 0 6px;padding:16px;background:#111;color:#fff;border-radius:16px}.agent-panel h2{margin:0 0 4px;font-size:18px}.agent-sub{font-size:12px;color:#bbb;margin-bottom:12px}.agent-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.agent{background:#1e1e1e;border:1px solid #343434;border-radius:12px;padding:12px}.agent-head{display:flex;justify-content:space-between;gap:10px;align-items:center}.agent-name{font-weight:800}.agent-state{font-size:11px;color:#aaa;margin-top:2px}.switch{position:relative;width:48px;height:27px;flex:none}.switch input{display:none}.track{position:absolute;inset:0;border-radius:99px;background:#555;cursor:pointer}.track:after{content:'';position:absolute;width:21px;height:21px;left:3px;top:3px;border-radius:50%;background:#fff;transition:.18s}.switch input:checked+.track{background:#8ee000}.switch input:checked+.track:after{transform:translateX(21px)}.aprogress{height:6px;background:#383838;border-radius:99px;overflow:hidden;margin-top:11px}.abar{height:100%;background:#fff}.agent-row{display:flex;justify-content:space-between;gap:8px;font-size:11px;margin-top:7px;color:#bbb}.agent-row b{color:#fff}.agent-error{font-size:11px;color:#ff8585;margin-top:7px;min-height:13px}.agent-actions{display:flex;gap:8px;margin-top:12px}.agent-actions button{border:1px solid #555;background:#222;color:#fff;border-radius:9px;padding:8px 10px;font-weight:700}.agent-actions button.primary{background:#fff;color:#111}
...
@media(max-width:900px){.agent-grid{grid-template-columns:1fr 1fr}}@media(max-width:700px){.shell{padding:14px}.controls{grid-template-columns:1fr 1fr}.controls input{grid-column:1/-1}.agent-grid{grid-template-columns:1fr}.agent-panel{margin-top:14px}}
</style>
</head>
<body><div class="shell"><div class="top"><h1>Vintage Evidence</h1><div class="sub">Sold surf / skate / street references with retained listing photography.</div><div class="stats" id="stats"></div><section class="agent-panel"><h2>Collectors</h2><div class="agent-sub">Independent 15-record workers. Switch any instance on or off; status refreshes every 5 seconds.</div><div id="agents" class="agent-grid"></div><div class="agent-actions"><button class="primary" onclick="setAll(true)">Start all</button><button onclick="setAll(false)">Stop all</button></div></section><div class="controls"><input id="q" placeholder="Search brand, title, tags…"><select id="brand"><option value="">All brands</option></select><select id="era"><option value="">All eras</option></select><select id="trad"><option value="">All traditions</option></select></div></div><main id="grid" class="grid"></main></div>
<script>
...
async function loadAgents(){try{const d=await fetch('/api/vintage-agents').then(r=>r.json());$('agents').innerHTML=(d.agents||[]).map(a=>{const pct=Math.min(100,Math.round(((a.batch_progress||0)/(a.batch_target||15))*100));return `<div class="agent"><div class="agent-head"><div><div class="agent-name">Agent ${a.id}</div><div class="agent-state">${esc(a.status)} · ${a.running?'running':'stopped'}</div></div><label class="switch"><input type="checkbox" ${a.enabled?'checked':''} onchange="toggleAgent(${a.id},this.checked)"><span class="track"></span></label></div><div class="aprogress"><div class="abar" style="width:${pct}%"></div></div><div class="agent-row"><span>Checkpoint</span><b>${a.batch_progress||0}/${a.batch_target||15}</b></div><div class="agent-row"><span>Completed</span><b>${a.completed_records||0} records</b></div><div class="agent-row"><span>Last listing</span><b>${esc(a.last_listing_id||'—')}</b></div><div class="agent-error">${esc(a.last_error||'')}</div></div>`}).join('')||'<div class="agent-error">Agent API unavailable.</div>'}catch(e){$('agents').innerHTML='<div class="agent-error">Agent controls unavailable.</div>'}}
