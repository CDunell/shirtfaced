...
const ENABLED=path.join(STATE,'enabled'), ATTEMPTED=path.join(STATE,'attempted-ids-v4.txt'), DISCOVERED=path.join(STATE,'discovered-ids.txt'), CURSOR=path.join(STATE,'discovery-cursor-v5.txt'), BUFFER=path.join(STATE,'checkpoint-buffer.jsonl');
const brands=['Stussy','Vision Street Wear','Powell Peralta','Santa Cruz','Independent Trucks','Thrasher','Airwalk','Alien Workshop','World Industries','Blind Skateboards','Hook Ups','Freshjive','Gotcha','Mambo','Quiksilver','Billabong','Rusty','Rip Curl','T&C Surf Designs','Ocean Pacific','Mossimo','No Fear','Volcom','Hobie','Hurley','DC Shoes','Element','Etnies','Zero Skateboards','Toy Machine','Girl Skateboards','Creature','RVCA','Anti Hero','Spitfire','Baker Skateboards','The Hundreds','Obey','LRG','Ecko','Famous Stars & Straps','Metal Mulisha','Plan B Skateboards','Independent','Vans','Emerica','Osiris','Globe','Sector 9','Lost Surfboards'];
const eras=['80s','90s','2000s','Y2K','vintage'];
const queryVariants=['shirt t-shirt','tee graphic','hoodie sweatshirt','cap hat','jersey crewneck','long sleeve'];
const pagesPerQuery=8;
const discoverySpace=brands.length*eras.length*queryVariants.length*pagesPerQuery;
...
const previousState=await readFile(path.join(STATE,'state.json'),'utf8').then(JSON.parse).catch(()=>({}));
let completed=Number(previousState.completed_records||0),batches=Number(previousState.completed_batches||0),lastListing=previousState.last_listing_id||null;
...
async function discover(minimum=60){
  ...
  for(let scanned=0;scanned<32&&added.length<minimum&&await enabled();scanned++,discoveryCursor++){
    if(discoveryCursor>=discoverySpace)discoveryCursor=0;
    const page=discoveryCursor%pagesPerQuery+1;
    const queryIndex=Math.floor(discoveryCursor/pagesPerQuery);
    const variantIndex=queryIndex%queryVariants.length;
    const eraIndex=Math.floor(queryIndex/queryVariants.length)%eras.length;
    const brandIndex=Math.floor(queryIndex/(queryVariants.length*eras.length))%brands.length;
    const brand=brands[brandIndex],eraName=eras[eraIndex],variant=queryVariants[variantIndex];
    const p=await context.newPage();
    try{
      const q=encodeURIComponent(`${brand} ${eraName} ${variant}`);
...
async function verify(id){const p=await context.newPage();try{await p.goto(`https://www.ebay.com/itm/${id}`,{waitUntil:'domcontentloaded',timeout:30000});await sleep(650);const body=await p.locator('body').innerText({timeout:5000}).catch(()=>''),html=await p.content().catch(()=>'');const evidence=`${body}\n${html}`;if(/pardon our interruption|security measure|access denied|captcha/i.test(body))return{reason:'blocked'};if(!/(This listing sold on|This item was sold|\bSOLD\b|Sold item|["'](?:itemStatus|listingStatus)["']\s*:\s*["']SOLD["'])/i.test(evidence))return{reason:'not-sold'};const title=((await p.title().catch(()=>''))||'').replace(/\s*\|\s*eBay.*$/i,'').trim();if(!/shirt|t-?shirt|tee|hoodie|sweatshirt|cap|hat|jersey|crewneck|polo|long sleeve/i.test(title))return{reason:'wrong-title'};const imgs=[...new Set(await p.locator('img').evaluateAll(xs=>xs.map(i=>i.currentSrc||i.src).filter(u=>/ebayimg\.com\/images\/g\//i.test(u||''))).catch(()=>[]))].slice(0,12);if(!imgs.length)return{reason:'no-images'};const d=path.join(IMG,id);await mkdir(d,{recursive:true});const prov=[];let n=0;for(const u0 of imgs){try{const u=u0.replace(/s-l\d+\.(jpg|jpeg|png|webp)$/i,'s-l1600.$1');const r=await context.request.get(u,{timeout:25000,headers:{referer:'https://www.ebay.com/'}});if(!r.ok())continue;const buf=await r.body();if(buf.length<3500)continue;n++;const file=`image-${String(n).padStart(2,'0')}.jpg`;await writeFile(path.join(d,file),buf);prov.push({file,source_url:u,source_kind:'ebay-rendered',sha256:crypto.createHash('sha256').update(buf).digest('hex'),byte_size:buf.length,acquired_at:new Date().toISOString()});}catch{}}if(!n)return{reason:'image-download'};const c=classify(title);const row={id:`EBAY-AGENT-${id}`,marketplace:'ebay',listing_id:id,sold:true,title,...c,era_claim:era(title),source_url:`https://www.ebay.com/itm/${id}`,retrieved:new Date().toISOString().slice(0,10),collector:`oracle-agent-${AGENT}`,stored_image_count:n};await writeFile(path.join(d,'record.json'),JSON.stringify(row,null,2));await writeFile(path.join(d,'provenance.json'),JSON.stringify(prov,null,2));return{row}}catch(error){return{reason:`exception:${error?.name||'unknown'}`}}finally{await p.close()}}
...
while(await enabled()){
  ...
  if(!candidates.length){
    ...
    candidates=[...new Set([...seedIds,...discovered])].filter(id=>shard(id)&&!seen.has(id)&&!attempted.has(id)&&!bufferedIds.has(id));
    if(!added&&!candidates.length){
      await state({status:'waiting-for-candidates',batch_progress:buffer.length,batch_target:TARGET,completed_batches:batches,completed_records:completed,last_listing_id:lastListing,last_error:null});
      await sleep(60000);
      continue;
    }
  }
  for(const id of candidates){
    ...
    if(result.row){buffer.push(result.row);bufferedIds.add(id);lastListing=id;await saveRows(BUFFER,buffer);await state({status:'collecting',batch_progress:buffer.length,batch_target:TARGET,completed_batches:batches,completed_records:completed,last_listing_id:id,last_error:null})}
    else{console.log(`REJECT ${id} ${result.reason}`);await state({status:'collecting',batch_progress:buffer.length,batch_target:TARGET,completed_batches:batches,completed_records:completed,last_listing_id:lastListing,last_error:`Rejected ${id}: ${result.reason}`})}
  }
  ...
  for(const row of batch)seen.add(String(row.listing_id));
...
await browser.close();
await state({status:'stopped',batch_progress:buffer.length,batch_target:TARGET,completed_batches:batches,completed_records:completed,last_listing_id:lastListing,last_error:null});
