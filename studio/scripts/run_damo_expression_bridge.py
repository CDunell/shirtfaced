...
def main():
 ...
 with get_session_factory()() as session:
  canonical=resolve_cast_reference(session,FilesystemAssetStore(settings.assets_root_resolved),slug="damo",role="head_shoulders_neutral")
 raw=canonical.data
 ...
 manifest={"character":"damo","experiment":"expression-bridge-roaring-chorus","generated_at":stamp,"model":model,"aspect_ratio":"4:5","image_size":"2K","canonical":canonical.as_manifest(),"canonical_sha256":canonical.sha256,"canonical_dimensions":list(dims),"change":["head_pose","expression"],"preserve":["identity","hair","stubble","age","skin","wardrobe"],"candidate_count":1,"manual_gate":"identity_bridge_review"}; (out/"manifest.json").write_text(json.dumps(manifest,indent=2)); (out/"prompt.txt").write_text(PROMPT); print(f"RESULT_DIR={out}")
