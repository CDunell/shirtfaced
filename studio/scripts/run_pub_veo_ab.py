...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--seed',required=True); p.add_argument('--arm',default='approved-master'); a=p.parse_args(); seed=Path(a.seed).resolve()
 ...
 stamp=datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ'); out=ROOT/'var/renderer-validation/pub-1105'/(stamp+'-veo-'+a.arm); out.mkdir(parents=True,exist_ok=True)
