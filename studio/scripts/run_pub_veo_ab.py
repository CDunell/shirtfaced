...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--seed',required=True); p.add_argument('--arm',default='approved-master')
 p.add_argument('--scene',default='pub-1105'); a=p.parse_args(); seed=Path(a.seed).resolve()
 if not seed.is_file(): raise SystemExit(f'missing seed: {seed}')
 lineage=scene_lineage(seed,a.scene); data=seed.read_bytes(); s=get_settings()
 ...
 stamp=datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ'); out=ROOT/'var/renderer-validation'/a.scene/(stamp+'-veo-'+a.arm); out.mkdir(parents=True,exist_ok=True)
 r=client.generate(GoogleVideoRequest(prompt=PROMPT,first_frame=data,first_frame_mime=mime,aspect_ratio='9:16',resolution=s.google_video_resolution)); raw=out/'video-generated.mp4'; raw.write_bytes(r.data); final=out/'video-1.mp4'
 # Strip generated audio deterministically; World 01 sound is authored in post.
 subprocess.run(['ffmpeg','-y','-i',str(raw),'-map','0:v:0','-c:v','copy','-an',str(final)],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL); raw.unlink()
 final_data=final.read_bytes(); m={**lineage,'experiment':'approved-master-direct-silent-veo','arm':a.arm,'generated_at':stamp,'model':r.model,'aspect_ratio':'9:16','resolution':s.google_video_resolution,'seed_path':str(seed),'seed_sha256':hashlib.sha256(data).hexdigest(),'video_sha256':hashlib.sha256(final_data).hexdigest(),'audio':'stripped_for_post','manual_gate':'keeper_segment_review'}; (out/'manifest.json').write_text(json.dumps(m,indent=2)); (out/'motion-prompt.txt').write_text(PROMPT); print(f'RESULT_DIR={out}')
