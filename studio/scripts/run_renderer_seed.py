...
def main():
 ...
 for i in range(1,args.candidates+1):
  result=client.generate(GoogleImageRequest(prompt=PUB_PROMPT,references=refs,aspect_ratio="9:16",image_size=args.image_size)); suffix=".png" if result.mime_type=="image/png" else ".jpg"; path=out/f"seed-{i}{suffix}"; path.write_bytes(result.data); print(f"generated {args.scene} candidate {i}: {path}")
