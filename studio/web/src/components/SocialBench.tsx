 ...
 *
 * No account connection and no publishing live here. This is the production
 * step immediately before Instagram/TikTok, and it remains useful when World
 * Builder later supplies the source asset automatically.
 */
...
function blobFromCanvas(canvas: HTMLCanvasElement): Promise<Blob> {
  return new Promise((resolve, reject) => {
    canvas.toBlob(
      (blob) => (blob ? resolve(blob) : reject(new Error("The browser could not encode the export."))),
      "image/jpeg",
      0.94,
    );
  });
...
export function SocialBench(): React.JSX.Element {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      setPhotos(found);
      if (found[0] && photoValue.length === 0) {
        setPhotoValue([{ id: found[0].id, label: found[0].label }]);
      }
    } catch (cause) {
  ...
  const go = useCallback(async () => {
    ...
    try {
      ...
      const chosenTheme = requested === "auto" ? analyseTheme(source) : requested;
      ...
      for (const spec of OUTPUTS.filter((item) => selectedOutputs.has(item.key))) {
        ...
        made.push({ ...spec, url: URL.createObjectURL(blob), filename, theme: chosenTheme });
      }
      setExports((old) => {
        old.forEach((item) => URL.revokeObjectURL(item.url));
        return made;
  ...
  return (
    <>
      <HeadingSmall marginTop={0} marginBottom={theme.sizing.scale300}>Social</HeadingSmall>
      <ParagraphSmall color={theme.colors.contentSecondary} marginTop={0}>
        Pick the asset. Pick the outputs. GO does the crop, contrast treatment, Shirtfaced layer and filenames.
      </ParagraphSmall>
      ...
      <div className={css({ display: "grid", gridTemplateColumns: "minmax(280px, 1.2fr) minmax(260px, .8fr)", gap: "16px", alignItems: "start", "@media screen and (max-width: 760px)": { gridTemplateColumns: "1fr" } })}>
        <Card>
          <StyledBody>
            <div className={css({ display: "flex", justifyContent: "space-between", gap: "8px", alignItems: "end", flexWrap: "wrap" })}>
              <div className={css({ flex: "1 1 280px" })}>
                <FormControl label="Source asset" caption="Approved World frames and anything uploaded to Studio">
                  <Select
                    clearable={false}
                    searchable
                    options={photos.map((item) => ({ id: item.id, label: item.label }))}
                    value={photoValue}
                    onChange={({ value }) => setPhotoValue(value)}
                  />
                </FormControl>
              </div>
              <Button size={SIZE.compact} kind={BUTTON_KIND.secondary} disabled={busy} onClick={() => uploadInput.current?.click()}>
                Upload photo
              </Button>
              <input ref={uploadInput} type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={(event) => { const file = event.currentTarget.files?.[0]; if (file) void onUpload(file); event.currentTarget.value = ""; }} />
            </div>

            {previewUrl ? (
              <div className={css({ marginTop: "8px", borderRadius: "16px", overflow: "hidden", backgroundColor: theme.colors.backgroundSecondary, aspectRatio: "4 / 5", display: "grid", placeItems: "center" })}>
                <img src={previewUrl} alt="Selected source" className={css({ width: "100%", height: "100%", objectFit: "cover" })} />
              </div>
            ) : (
              <div className={css({ padding: "48px 16px", textAlign: "center", color: theme.colors.contentSecondary })}>No source asset yet.</div>
            )}

            {photo ? (
              <div className={css({ display: "flex", flexWrap: "wrap", gap: "6px", marginTop: "10px" })}>
                <Tag closeable={false} kind={TAG_KIND.neutral}>{photo.width}×{photo.height}</Tag>
                <Tag closeable={false} kind={TAG_KIND.neutral}>{photo.uploaded ? "uploaded" : "world asset"}</Tag>
                {photo.from_prompt ? <Tag closeable={false} kind={TAG_KIND.neutral}>{photo.from_prompt.shot_external_id} / v{photo.from_prompt.variation}</Tag> : null}
              </div>
            ) : null}
          </StyledBody>
        </Card>
        ...
        <Card>
          <StyledBody>
            <FormControl label="Contrast treatment" caption="Auto samples the actual image; it does not assume nightlife.">
              <Select clearable={false} options={THEME_OPTIONS} value={themeValue} onChange={({ value }) => setThemeValue(value)} />
            </FormControl>
            <FormControl label="Branding">
              <Select clearable={false} options={BRANDING_OPTIONS} value={brandingValue} onChange={({ value }) => setBrandingValue(value)} />
            </FormControl>
            ...
            <div className={css({ marginTop: "8px", marginBottom: "16px" })}>
              {OUTPUTS.map((spec) => (
                <Checkbox
                  ...
                  onChange={() => setSelectedOutputs((old) => { const next = new Set(old); if (next.has(spec.key)) next.delete(spec.key); else next.add(spec.key); return next; })}
                >
                  {spec.label} <span className={css({ color: theme.colors.contentTertiary })}>{spec.width}×{spec.height}</span>
                </Checkbox>
              ))}
            </div>

            <Button disabled={!photo || selectedOutputs.size === 0} isLoading={busy} onClick={() => void go()} overrides={{ BaseButton: { style: { width: "100%", minHeight: "52px", fontSize: "18px", fontWeight: 700 } } }}>
              GO
            </Button>
      ...
      {exports.length > 0 ? (
        <div className={css({ marginTop: theme.sizing.scale800 })}>
          <div className={css({ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "8px", flexWrap: "wrap" })}>
            <div>
              <HeadingSmall marginBottom={0}>Ready</HeadingSmall>
              <ParagraphXSmall color={theme.colors.contentSecondary} marginTop={0}>
                {resolvedTheme ? `Resolved ${resolvedTheme}. ` : ""}{exports.length} export{exports.length === 1 ? "" : "s"}.
              </ParagraphXSmall>
            </div>
            <Button size={SIZE.compact} kind={BUTTON_KIND.secondary} onClick={downloadAll}>Download all</Button>
          </div>
          <div className={css({ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: "12px", marginTop: "10px" })}>
            {exports.map((item) => (
              <Card key={item.key}>
                <StyledBody>
                  <img src={item.url} alt={item.label} className={css({ width: "100%", aspectRatio: `${item.width} / ${item.height}`, objectFit: "cover", borderRadius: "10px", display: "block", marginBottom: "8px" })} />
                  <LabelSmall>{item.label}</LabelSmall>
                  <ParagraphXSmall color={theme.colors.contentSecondary}>{item.filename}</ParagraphXSmall>
                  <a href={item.url} download={item.filename} className={css({ color: "inherit", textDecoration: "none" })}>
